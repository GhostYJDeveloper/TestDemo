...
<div class="layui-fluid">
    ...
    <div class="layui-card-body">
        ...
        <script type="text/html" id="table-action">
            ...
            <a class="layui-btn layui-btn-normal layui-btn-xs layui-btn-warm" lay-event="order">下单</a>
...
<script>
    layui.config({
        ...
        table.render({
            elem: '#lay-table',
            url: layui.setter.contextPath + '/rest/warehouse',
            cols: [[
                {type: 'checkbox', fixed: 'left'},
                {field: 'id', title: 'Id', align: 'center'},
                {field: 'number', title: '货物编号', align: 'center'},
        ...
        table.on('tool(lay-table)', function (obj) {
            ...
            if (obj.event === 'detail') {
                var tr = $(obj.tr);
                $.ajax({
                    url: layui.setter.contextPath + '/rest/warehouse/' + obj.data.id,
                    type: 'GET',
                    dataType: 'JSON',
                ...
                layer.confirm('确定删除此数据？', function (index) {
                    $.ajax({
                        url: layui.setter.contextPath + '/rest/warehouse?id=' + obj.data.id,
                        type: 'DELETE',
                        dataType: 'JSON',
                        success: function (data) {
                            if (data.success) {
        ...
        var active = {
            batchDelete: function () {
                ...
                layer.confirm('确定删除吗？', function (index) {
                    $.ajax({
                        url: layui.setter.contextPath + '/rest/warehouse/batch-delete',
                        type: 'DELETE',
                        dataType: 'JSON',
